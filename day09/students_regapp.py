...
from tkinter.font import * # 기본 외의 폰트를 사용하기 위함

import pymysql  # mysql-connector 등 다른 모듈도 사용 추천
...
def showDatas():
    ...
    ### DB연결. 커넥션 객체생성 -> 커서 -> 쿼리실행 -> 커서로 데이터 패치 -> 커서 종료 -> 커넥션 종료
...
def getData(event):
    '''
    트리뷰 더블클릭으로 선택된 학생 정보를 Entry 위젯에 채우는 사용자 정의 함수
...
def addData():
    ...
    ## DB연결
    conn = pymysql.connect(host=host, user=username, passwd=password, port=port, db=database)
    cursor = conn.cursor() # DB 쿼리 실행 시 커서 생
...
def delData():
    ...
    if std_id == '':
        messagebox.showinfo('DELETE', '삭제 데이터 선택요망!')
        return
