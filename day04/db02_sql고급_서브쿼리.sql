...
-- 주문 번호와 판매금액을 보이시오.
-- 비교연산자 사용 - 서브쿼리의 값이 단일값이 되어야 함!! (제약사항 생김!)
...
-- ALL|SOME|ANY를 쓰면 서브쿼리의 값이 단일값이 아니여도 상관 없음
...
-- 추가. 최신 방법(서브쿼리에서 두 가지 컬럼을 비교하는 방법) - 튜플(파이썬과 동일)
-- 2010년대 중반 이후부터 사용 예상
SELECT *
  FROM Orders
...
-- SELECT 서브쿼리
-- 4-17  고객별 판매액을 나타내시오. (고객 이름과 고객별 판매액 출력)
SELECT o.custid
...
-- 이 테이블이 하나의 가상의 테이블이 된다.
SELECT custid
	 , name
  FROM Customer
 WHERE custid <= 2;
 
 -- 2. 위의 가상의 테이블을 cs 라고 이름짓고, FROM에 넣어줌
 SELECT cs.*
   FROM (SELECT custid
			  , name
		   FROM Customer
