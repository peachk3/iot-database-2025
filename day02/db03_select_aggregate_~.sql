...
-- GROUP BY를 사용하면 반드시 집계함수 및 GROUP BY에 포함된 컬럼이 SELECT 안에 들어가있어야 함
-- GROUP BY에 있는 컬럼만 SELECT 안에 사용할 수 있음. 그 외는 사용 불가
SELECT custid
...
SELECT custid
 ...
 GROUP BY custid
HAVING COUNT(*) >= 2; -- COUNT(*)와 별명을 사용할 수 있음
...
-- 외부조인 : 조건을 만족하지 않는(일치하지 않는) 데이터도 출려이 필요할 때 사용하는 조인
...
-- 대한미디어에서 출판한 책(bookid), 그 책을 구매한 고객(custid), 고객 이름(name)
SELECT name
  FROM Customer
 WHERE custid IN(
	SELECT custid
...
-- EXISTS : 상관서브쿼리에서 사용하는 키워드. 상관서브쿼리가 아니라도 사용 가능
