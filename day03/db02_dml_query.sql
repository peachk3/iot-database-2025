-- INSERT
-- 3-44 Book테이블에 '스포츠 의학' 도서를 추가하시오. 한솔의학서적에서 출간했고, 90,000원이다.
...
INSERT INTO Book VALUES (12, '에벤져스 스토리', '디즈니', 50000); -- Duplicate 에러 발생
...
-- 3-46 imported_book에 Book 테이블의 데이터를 모두 삽입하시오.
...
CREATE TABLE NewBook (
	bookid 		integer 	primary key auto_increment, --  auto_increment는 숫자 자동증가 조건
    bookname 	varchar(50) not null,
    publisher 	varchar(50) not null,
    price 		int			null -- null은 생략 가능
);

-- 자동증가에는 PK 컬럼 사용하지 않음
...
-- UPDATE 
-- 3-47 Customer 테이블에서 고객번호가 5번인 고객의 주소를 '대한민국 부산'으로 변경하시오.
...
-- 3-48 Book 테이블의 14번 '스포츠 의학'의 출판사를 imported_book 테이블에 있는 21번 책의 출판사와 동일하게 변경
-- 1. Book 테이블에 14번의 기존 데이터 확인
SELECT * FROM Book WHERE bookid = 14;

-- 2. Imported_Book 테이블에 21번의 기존 데이터 확인
SELECT * FROM Imported_Book WHERE bookid = 21;

-- 3. update 쿼리 작성
UPDATE Book
   SET bookname = '스포츠 의학'
	, publisher = (select publisher from Imported_Book where bookid = 21)
 WHERE bookid = 14;

-- 추가. 데이터 수정시 조심할 것
SELECT * FROM NewBook;

-- WHERE 절 없이 UPDATE 하지 말 것
UPDATE NewBook
   SET price = 100000
WHERE bookid = 4;
