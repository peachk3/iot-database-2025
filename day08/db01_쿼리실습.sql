...
/* 프로젝트 성공으로 급여인상이  결정됨
   사원 현재 107명 19개 업무에 소속되어 근무중. 회사 업무 Distnict job_id  5개 업무에서 일하는 사원
   HR_REP(10), MK_REP(12), PR_REP(15), SA_REP(18), IT_PROG(20) 5개 업무를 제외하고는 나머지는 동력 (107)
*/
select employee_id
...
/* 월별로 입사한 사원 수가 아래와 같이 행별로 출력되도록 하시오. (12행)
*/
select '09'; -- string
-- 형변환 함수 case(), convert()
select cast('-09' as unsigned); -- unsigned(양수만 표현하는 숫자형)
...
/* Error Code: 1055. Expression #2 of SELECT list is not in GROUP BY clause and contains nonaggregated column 'hr.employees.hire_date' which is not functionally dependent on columns in GROUP BY clause; 
...
/* 부서와 업무별 급여합계를 구하고 신년도 급여수준 레벨을 지정하려고 함
   부서 번호와 업무를 기준으로 행을 그룹별로 나누어 급여합계와 인원수를 출력 (20개행)
*/
...
select department_id
  ...
  from employees
 group by department_id, job_id
  with rollup; -- group by의 컬럼이 하나면 총계는 하나, 컬럼이 두 개면 첫번째 컬럼별로 소계, 두 컬럼의 합산이 총계로 나옴

/* 이전 문제를 활용, 집계 결과가 아니면 (ALL-DEPTs)라고 출력, 업무에 대한 집계결과가 아니면(ALL-JOBs)를 출력
   rollup 으로 만들어진 소계면 (ALL-JOBs), 총계면 (ALL-DEPTs)
*/
...
-- RANK
/* 분석함수 NTILE()을 사용, 부서별 급여 합계를 구하시오. 급여가 제일 큰 것이 1, 제일 작은 것이 4가 되도록 등급을 나눔(12행)
*/
select department_id
...
select employee_id
     ...
     , salary
     , department_id 
     , rank() over (partition by department_id order by salary desc) as 'Rank' -- 1, 1, 3, 4 순위 매기기
	 , dense_rank() over (partition by department_id order by salary desc) as 'Dense_Rank' -- 1, 1, 2, 3 순위 매기기
     , row_number() over (partition by department_id order by salary desc) as 'Row_Number' -- 1부터 단순한 행 번호 매기기
  from employees
